...
def generate_raw_fpath(
    ...
    subject : str
    -   The name of the subject for which the mne_bids.BIDSPath object should
        be generated.

    session : str
    -   The name of the session for which the mne_bids.BIDSPath object should
        be generated.
...
def generate_fpath_from_analysed(
    ...
    -   Each dictionary contains the following keys regarding what the
        ...
        'sub' for the subject's name; 'ses' for the session name; 'task' for
        the task name; 'acq' for the acquisition name; 'run' for the run name.
    -   If multiple types of a single attribute (e.g. 'cohort', 'sub', etc...)
        are present, then these values will be replaced with "multi" to
        indicate the information has been derived from multiple sources.
...
def check_annots_orig_time(annots: mne.Annotations) -> mne.Annotations:
    ...
    in the MNE Annotations object, setting it to 'None' if this is not the
    case.

...
class ProcConnectivity(ProcMethod):
    ...
    def _sort_inputs(self) -> None:
        ...
        if self.signal._data_dimensions != supported_data_dims:
            raise ValueError(
                ...
                f"{self.signal.data_dimensions}, but only data with "
                f"dimensions {supported_data_dims} is supported."
            )
        super()._sort_inputs()
    ...
    def _take_absolute_connectivity(
        self,
        connectivity: list[
            Union[SpectralConnectivity, SpectroTemporalConnectivity]
        ],
        ...
        for con_class in connectivity:
            if isinstance(con_class, SpectroTemporalConnectivity):
                ...
                raise TypeError(
                    "The connectivity object provided is not of a supported "
                    "type."
                )
    ...
    def _average_windows_results(self) -> None:
        ...
        for con_method in self.results:
            kwargs = dict(
                data=np.asarray([data.get_data() for data in con_method]).mean(
                    axis=0
                ),
    ...
    def _average_timepoints_results(self) -> None:
        ...
        for con_method in self.results:
            ...
            for window_data in con_method:
                ...
                time_averaged_results[-1].append(
                    SpectralConnectivity(**kwargs)
                )
    ...
    def _dimensions_info_for_results_dict(self) -> dict:
        """Returns information about the dimensions of the results.
...
class ProcMultivariateConnectivity(ProcConnectivity):
    ...
    def _sort_seeds_targets(self) -> None:
        ...
        If the seeds and/or targets are dictionaries, the names of the seeds
        and targets will be automatically generated based on the information in
        the dictionaries, and then expanded, such that connectivity is
        calculated between every seed and every target.
        ...
        for group_i, group in enumerate(groups):
            ...
            if isinstance(group_vals, dict):
                ...
                raise TypeError(
                    "Seeds and targets must given as lists, or as "
                    "dictionaries with instructions for generating these "
                    f"lists, however the {group} are of type "
                    f"{type(group_vals)}."
                )
        ...
        if isinstance(self.n_components, (list, tuple)):
            ...
            if all(isinstance(n_comps, int) for n_comps in self.n_components):
                ...
                for i in range(2):
                    n_components[i] = [
                        self.n_components[i]
                        for _ in range(len(self.indices[i]))
                    ]
                self.n_components = tuple(n_components)
            elif not all(
                isinstance(n_comps, list) for n_comps in self.n_components
            ):
                raise TypeError(
                    "`n_components` must be a list of lists of ints or a list "
                    "of ints."
                )
        elif self.n_components is not None and not isinstance(
            self.n_components, str
        ):
    ...
    def _check_connectivity_classes(
        ...
        for con_class in connectivity:
            ...
            if not isinstance(
                ...
                con_class, MultivariateSpectroTemporalConnectivity
            ):
                correct_class = False
    ...
    def _generate_node_ch_reref_types(self) -> None:
        """Gets the rereferencing types of channels in the results.

        If the rereferencing types of each channel in a seed/target for a given
        node are identical, this type is given as a string, otherwise the
        unique types are taken and joined into a single string by the " & "
        characters.
        """
        ch_reref_types = {}
        for ch_i, combined_name in enumerate(self._comb_names_str):
    ...
    def _generate_node_ch_subregions(self) -> None:
        ...
        for node_i, combined_name in enumerate(self._comb_names_str):
            ...
            ch_subregions[combined_name] = combine_vals_list(
                unique(subregions)
            )
    ...
    def _generate_node_lateralisation(
        ...
        for node_i in range(len(node_ch_hemispheres[0])):
            if (
                node_ch_hemispheres[0][node_i]
                != node_ch_hemispheres[1][node_i]
            ):
                if (

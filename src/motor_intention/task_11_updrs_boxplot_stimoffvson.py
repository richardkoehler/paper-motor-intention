...
BASENAME = "UPDRS_boxplot_stimoffvson"
...
def task_plot_updrs_medoffvson() -> None:
    ...
    data_raw = pd.read_csv(PART_INFO).rename(
        columns={
            ...
            f"{y} postopDBSON": Cond.ON_STN_DBS.value,
        }
    )
    data = (
        pd.melt(
        ...
        .query(
            f"Subject in {[sub.strip('sub-') for sub in constants.STIM_PAIRED_SUBS]}"
        )
        .sort_values("Subject")
    )
    print(data.head())
    ...
    with open(FNAME_STATS, "w", encoding="utf-8", newline="") as file:
        ...
        for cond_a, cond_b in ((Cond.OFF_THERAPY, Cond.ON_STN_DBS),):
